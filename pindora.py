...
def main():
    """Example usage of Pindora drug discovery pipeline"""
    print("=" * 60)
    print("Pindora Drug Discovery Pipeline")
    print("=" * 60)
    
    # Initialize Pindora
    pindora = Pindora()
    
    # Example: Disease prompt
    disease_prompt = "i have breast cancer and diabetes type 2"
    
    print(f"\nProcessing disease prompt: '{disease_prompt}'")
    print("-" * 60)
    
    # Run drug discovery pipeline
    results = pindora.drug_discovery_pipeline(disease_prompt)
    
    print("\n" + "=" * 60)
    print(f"Pipeline completed! Generated {len(results)} drug candidates")
    print("=" * 60)
    print(f"\nResults saved to: data/generated_molecules_new.json")
    
    # Show summary
    if results:
        print(f"\nSummary:")
        print(f"  Total unique drugs with variants: {len(results)}")
        total_variants = sum(len(drug['generated_molecules']) for drug in results)
        print(f"  Total generated variants: {total_variants}")
        
        print(f"\nSample results (first 3):")
        for idx, drug in enumerate(results[:3], 1):
            print(f"\n  {idx}. Drug: {drug['drug_name']}")
            print(f"     Disease: {drug['disease_name']}")
            print(f"     Target: {drug['target_symbol']}")
            print(f"     Input SMILES: {drug['input_smile']}")
            print(f"     Generated variants: {len(drug['generated_molecules'])}")
